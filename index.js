/**
 * Original Author: Alex Ford (Chevex)
 * This is only overwritten to add async/await support
 * because the original code was written with callbacks
 * that has been deprecated in version 5 of mongoose.
 */
...
exports.plugin = async function (schema, options) {
    ...
    let counter;
    try {
        counter = await IdentityCounter.findOne({ model: settings.model, field: settings.field });
    } catch (err) {
        throw err;
    }
